const { friendModel, activityModel, todoModel } = require("../Utils/Schemas.js");
...
exports.run = async (request, ActorId) => {
  if (request.friendId != 1) {
    await friendModel.updateOne({ ReceiverId: ActorId, RequesterId: request.friendId }, { Status: 0 });
    await friendModel.updateOne({ RequesterId: ActorId, ReceiverId: request.friendId }, { Status: 0 });

    await activityModel.updateMany({ ActorId: request.friendId, Friend: ActorId, Type: 2 }, { ActorId: 0, FriendId: 0 });
