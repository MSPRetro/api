...
exports.run = async (request, ActorId) => {
  if (isNaN(request.report.Comment && typeof request.report.Comment !== "string")) request.report.Comment = "";
  
  if (!await userModel.findOne({ ActorId: request.report.ReportedActorId })) return;
  if (request.report.MovieId !== 0 && !await movieModel.findOne({ MovieId: request.report.MovieId })) return;
