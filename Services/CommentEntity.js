const { lookModel, competitionModel, commentEntityModel } = require("../Utils/Schemas.js");
...
exports.run = async (request, ActorId) => {
  switch (request.entityComment.EntityType) {
    case 1:
      const look = await lookModel.findOne({ LookId: request.entityComment.EntityId, State: 0 });
      if (!look) return;
      ...
      break;
    case 5:
      const competition = await competitionModel.findOne({ MovieCompetitionId: request.entityComment.EntityId });
      if (!competition) return;

      let EntityCommentIdCompetition = await getNewId("comment_entity_id") + 1;
      ...
      break;
    default:
      return console.log(`[CommentEntity] : ${request.entityComment.EntityType} in not coded.`);
  };
