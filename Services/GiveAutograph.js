...
exports.run = async (request, ActorId) => {  
  ...
  const receiverActorId = await userModel.findOne({ ActorId: request.receiverActorId });
  ...
  if (!receiverActorId || new Date(addMinutes(new Date(user.Autographs.TimeOfLastAutographGiven), timeBeforeNewAuto)).getTime() > Date.now()) return;
  
  await userModel.updateOne({ ActorId: ActorId }, { $set: {
    "Autographs.NumberOfAutographsGiven": user.Autographs.NumberOfAutographsGiven + 1,
