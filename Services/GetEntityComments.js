...
exports.run = async request => {
  ...
  switch (request.EntityType) {
    case "look":
      ...
      const commentsLook = await commentEntityModel.find(json)
      .sort({ _id: -1 })
      .skip(request.pageindex * 3)
      .limit(3);
      
      for (let comment of commentsLook) {        
        const user = await userModel.findOne({ ActorId: comment.ActorId });
    ...
    case "moviecompetition":
      ...
      const commentsCompetition = await commentEntityModel.find(json)
      .sort({ _id: -1 })
      .skip(request.pageindex * 3)
      .limit(3);
      ...
      break;
    default:
      return console.log(`[GetEntityComments] : ${request.EntityType} in not coded.`);
  };
