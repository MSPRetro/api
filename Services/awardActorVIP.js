...
const { buildXML, addDays, numStr, getNewId } = require("../Utils/Util.js");
...
exports.run = async request => {
  ...
  if (timeout == 0 || Date.now() > timeout) timeout = new Date(addDays(new Date(), request.days++));
  else timeout = new Date(addDays(user.VIP.MembershipTimeoutDate, request.days++));
