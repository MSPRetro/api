...
exports.run = async (request, ActorId) => {
  const gift = await giftModel.findOne({ GiftId: request.giftId, ReceiverActorId: ActorId, State: 0 });
  if (!gift) return;
  
  await giftModel.updateOne({ GiftId: request.giftId, ReceiverActorId: ActorId, State: 0 }, {
    State: 1
  });
  
  await idModel.updateOne({ ClothesRellId: gift.ClothesRellId }, {
    ActorId: ActorId
  });
